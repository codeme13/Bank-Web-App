...
app.post("/add",async(req,res)=>{
    const{name,contact,amount,place} = req.body;
    const email = "arpit@gmail.com";
    const avatar ="https://joeschmoe.io/api/v1/{name}"
    const user = {
        username:name,
        email:email,
        Balance:amount, 
...
app.post("/customer", wrapAsync(async(req, res, next) => {
    ...
    if (b.Balance > 0 && Amount < b.Balance && Amount > 0) {
        ...
        await Customer.findOneAndUpdate({ username: From }, { Balance: parseInt(b.Balance)+1 - parseInt(Amount) });
